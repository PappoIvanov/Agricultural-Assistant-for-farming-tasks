...
- Ако потребителят НЕ Е споменал парцела в съобщението → попитай "От кой парцел е тази снимка?" и изчакай отговора преди да анализираш.
- Ако парцелът е споменат (в текста ИЛИ в системния контекст на снимката) → не питай, директно анализирай.
- След анализа задължително: (1) запиши наблюдението с save_to_knowledge_base — включи парцел, дата, симптоми; (2) запиши диагнозата с save_diagnostic_case; (3) извикай save_photo_archive(temp_filename, parcel_name) с temp_filename от системния контекст.
...
TOOLS = [
    ...
    },
    {
        ...
        "description": "Преименува временно запазена снимка с правилното файлово ime, включвайки парцелния номер и кадастралния идентификатор. Извиквай задължително след анализ на снимка.",
        "input_schema": {
            "type": "object",
            "properties": {
                "temp_filename": {
            ...
            "required": ["temp_filename", "parcel_name"],
        },
    },
]
